...
namespace detail
{
                                                    ...
                                                    bool B_transposed, bool B_row_major, bool C_row_major)
  {
    ...
    /** @brief Generate kernel for C = A * B' with A, C being dense, B is sparse (Author: Sangkyun Lee) */
    template<typename StringT>
    void generate_csr_dense_trans_sparse_matrix_mult(StringT & source, std::string const & numeric_string, bool C_row_major)
    {
      source.append("__kernel void csr_dense_trans_sparse_matrix_mult");
      source.append("( \n");
      source.append("  __global const unsigned int * sp_mat_row_indices, \n");
      source.append("  __global const unsigned int * sp_mat_col_indices, \n");
      source.append("  __global const "); source.append(numeric_string); source.append(" * sp_mat_elements, \n");
      source.append("  unsigned int sp_mat_nnz, \n");
      source.append("  __global const "); source.append(numeric_string); source.append(" * d_mat, \n");
      source.append("  unsigned int d_mat_row_start, \n");
      source.append("  unsigned int d_mat_col_start, \n");
      source.append("  unsigned int d_mat_row_inc, \n");
      ...
      source.append("      unsigned int col_start = sp_mat_row_indices[col]; \n"); // treat sp_mat as it were transposed
      source.append("      unsigned int col_end = sp_mat_row_indices[col+1]; \n");

      source.append("      for (unsigned int k = col_start; k < col_end; k ++) { \n");

      source.append("        unsigned int j = sp_mat_col_indices[k]; \n");
      source.append("        ");
      source.append(numeric_string); source.append(" x = d_mat[ (d_mat_row_start +   row * d_mat_row_inc) * d_mat_internal_cols + d_mat_col_start + j * d_mat_col_inc ]; \n");
      source.append("        ");
      source.append(numeric_string); source.append(" y = sp_mat_elements[k]; \n");
    ...
    /** @brief Generate kernel for C = A * B with A, C being dense, B is sparse (Author: Sangkyun Lee) */
    template<typename StringT>
    void generate_csr_dense_sparse_matrix_mult(StringT & source, std::string const & numeric_string, bool C_row_major)
    {
        source.append("__kernel void csr_dense_sparse_matrix_mult");
        source.append("( \n");
        source.append("  __global const unsigned int * sp_mat_row_indices, \n");
        source.append("  __global const unsigned int * sp_mat_col_indices, \n");
        source.append("  __global const "); source.append(numeric_string); source.append(" * sp_mat_elements, \n");
        ...
        // split work rows (sparse matrix rows) to thread groups
        source.append("  for (unsigned int col = get_group_id(0); col < result_col_size; col += get_num_groups(0)) { \n");

        // split result cols between threads in a thread group
        source.append("    for ( unsigned int row = get_local_id(0); row < result_row_size; row += get_local_size(0) ) { \n");

        source.append("      "); source.append(numeric_string); source.append(" r = 0; \n");
        source.append("        unsigned int row_idx = 0; \n");
        source.append("      for (unsigned int k = 0; k < sp_mat_nnz; k++) { \n");
        source.append("        if(row_idx < (d_mat_col_size-1) && k == sp_mat_row_indices[row_idx+1]) row_idx++; \n");
        source.append("        if(col == sp_mat_col_indices[k]) { \n");
        source.append(numeric_string); source.append(" x = d_mat[ (d_mat_row_start +   row * d_mat_row_inc) * d_mat_internal_cols + d_mat_col_start + row_idx * d_mat_col_inc ]; \n");
        source.append("        ");
        source.append(numeric_string); source.append(" y = sp_mat_elements[k]; \n");
        source.append("           r += x * y; \n");
        source.append("           k = sp_mat_row_indices[++row_idx]-1; \n");
        source.append("        }");
        source.append("      } \n");
...
//        source.append("      "); source.append(numeric_string); source.append(" r = 0; \n");
//        //source.append("      unsigned int row_idx = 0; \n");
//        source.append("      for (unsigned int l = 0; l < d_mat_col_size; ++l ) { \n");
//        source.append("      for (unsigned int k = sp_mat_row_indices[l]; k < sp_mat_row_indices[l+1]; ++k) { \n");
//        source.append("        if(col == sp_mat_col_indices[k]) { \n");
//        source.append(numeric_string); source.append(" x = d_mat[ (d_mat_row_start +   row * d_mat_row_inc) * d_mat_internal_cols + d_mat_col_start + l * d_mat_col_inc ]; \n");
//        source.append("        ");
//        source.append(numeric_string); source.append(" y = sp_mat_elements[k]; \n");
//        source.append("           r += x * y; \n");
//        //source.append("           break; \n");
...
//    }
    ...
    /** @brief Generate kernel for C = A * B with A, C being dense, B is sparse (Author: Sangkyun Lee) */
    template<typename StringT>
    void generate_copy_csr_to_dense(StringT & source, std::string const & numeric_string, bool C_row_major)
    {
        source.append("__kernel void copy_csr_to_dense");
        source.append("( \n");
        source.append("  __global const unsigned int * sp_mat_row_indices, \n");
        source.append("  __global const unsigned int * sp_mat_col_indices, \n");
        source.append("  __global const "); source.append(numeric_string); source.append(" * sp_mat_elements, \n");
        ...
        source.append("      for (unsigned int k = row_start + get_local_id(0); k < row_end; k+= get_local_size(0) ) { \n");

        source.append("        unsigned int col = sp_mat_col_indices[k]; \n");
        source.append("        result[ (result_row_start + row * result_row_inc) * result_internal_cols + result_col_start + col * result_col_inc ] =  sp_mat_elements[k]; \n");
...
void generate_compressed_matrix_dense_matrix_multiplication(StringT & source, std::string const & numeric_string)
{
  ...
  detail::generate_csr_dense_trans_sparse_matrix_mult(source, numeric_string, true);
  //detail::generate_csr_dense_trans_sparse_matrix_mult(source, numeric_string, false);
  detail::generate_csr_dense_sparse_matrix_mult(source, numeric_string, true);
  //detail::generate_csr_dense_sparse_matrix_mult(source, numeric_string, false);
  detail::generate_copy_csr_to_dense(source, numeric_string, true);
}
