                                       ...
                                       std::string const & funcname, std::string const & op, std::string const & op_name)
{
  ...
  source.append("    uint4 size2,double gamma) { \n");
  ...
  else if (funcname == "prox")
  {
    source.append("if(vec1[i*size1.y+size1.x] >gamma*0.001)\n");
    source.append("{\n");
    source.append("vec1[i*size1.y+size1.x]-=gamma*0.001;\n");
    source.append("}\n");
    source.append("else if(vec1[i*size1.y+size1.x] <-gamma*0.001)\n");
    source.append("{\n");
    source.append("vec1[i*size1.y+size1.x]+=gamma*0.001;\n");
    source.append("}\n");
    source.append("else\n");
    source.append("{\n");
    source.append("vec1[i*size1.y+size1.x]=0.0;\n");
    source.append("}\n");
  }
  else if (funcname == "element_product")
  {
    source.append("if(vec2[i*size2.y+size2.x]==0.0){\n");
    source.append(" vec1[i*size1.y+size1.x] = 0;\n");
    source.append("}\n");
  }
  else
  {
...
struct vector_element
{
  ...
  static void init(viennacl::ocl::context & ctx)
  {
    ...
    if (!init_done[ctx.handle().get()])
    {
      ...
      {
        generate_vector_unary_element_ops(source, numeric_string, "prox");
        generate_vector_unary_element_ops(source, numeric_string, "element_product");
