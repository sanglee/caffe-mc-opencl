                                       ...
                                       std::string const & funcname, std::string const & op, std::string const & op_name, bool is_row_major)
{
  ...
  source.append("          unsigned int B_internal_size1,  unsigned int B_internal_size2, double gamma) { \n");
...
//    source.append("if(A[(row * A_inc1 + A_start1) * A_internal_size2 + col * A_inc2 + A_start2] >gamma*0.001)\n");
//    source.append("{\n");
//    source.append("A[(row * A_inc1 + A_start1) * A_internal_size2 + col * A_inc2 + A_start2]-=gamma*0.001;\n");
//    source.append("}\n");
//    source.append("else if(A[(row * A_inc1 + A_start1) * A_internal_size2 + col * A_inc2 + A_start2] <-gamma*0.001)\n");
//    source.append("{\n");
//    source.append("A[(row * A_inc1 + A_start1) * A_internal_size2 + col * A_inc2 + A_start2]+=gamma*0.001;\n");
//    source.append("}\n");
//    source.append("else\n");
//    source.append("{\n");
//    source.append("A[(row * A_inc1 + A_start1) * A_internal_size2 + col * A_inc2 + A_start2]=0.0;\n");
//    source.append("}\n");
    source.append("for (unsigned int row = row_gid; row < A_size1; row += get_num_groups(0)){\n");
    source.append("   for (unsigned int col = col_gid; col < A_size2; col += get_local_size(0)){\n");
//    source.append("     A[(row * A_inc1 + A_start1) * A_internal_size2 + col * A_inc2 + A_start2] = min(max(A[(row * A_inc1 + A_start1) * A_internal_size2 + col * A_inc2 + A_start2]-gamma*0.001,0.0),A[(row * A_inc1 + A_start1) * A_internal_size2 + col * A_inc2 + A_start2]+gamma*0.001);\n");
    source.append("__global float *element = &A[(row * A_inc1 + A_start1) * A_internal_size2 + col * A_inc2 + A_start2];\n");
    source.append("*element = min((float)(max((float)(*element-gamma*0.001),(float)(0.0))),(float)(*element+gamma*0.001));\n");
//    source.append("A[(row * A_inc1 + A_start1) * A_internal_size2 + col * A_inc2 + A_start2] = element;\n");

  source.append(" }\n}\n");
   }
  else if(funcname=="element_product"){
	source.append("  for (unsigned int row = row_gid; row < A_size1; row += get_num_groups(0)) \n");
    source.append("    for (unsigned int col = col_gid; col < A_size2; col += get_local_size(0)){ \n");
       source.append("if(B[(row * B_inc1 + B_start1) * B_internal_size2 + col * B_inc2 + B_start2]==0.0){\n");
       source.append(" A[(row * A_inc1 + A_start1) * A_internal_size2 + col * A_inc2 + A_start2] = 0;\n");
        source.append("}\n");
	source.append("}\n");
}  
 else{
...
struct matrix_element
{
  ...
  static void init(viennacl::ocl::context & ctx)
  {
    ...
    if (!init_done[ctx.handle().get()])
    {
      ...
      {
        generate_matrix_unary_element_ops(source, numeric_string, "prox",  is_row_major);
        generate_matrix_unary_element_ops(source, numeric_string, "element_product", is_row_major);
