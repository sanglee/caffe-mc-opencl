...
/** @brief Carries out dense-sparse_matrix multiplication second matrix being compressed and transposed
*
* Author: Sangkyun Lee
* Implementation of the convenience expression y = prod(d_A, sp_A);
*
* @param d_A      The dense matrix
* @param sp_A     The sparse matrix
* @param y        The y matrix
* @param trans    transpose sp_A
*/
template< typename NumericT, unsigned int AlignmentV>
void prod_impl(const viennacl::matrix_base<NumericT> & d_A,
               const viennacl::compressed_matrix<NumericT, AlignmentV> & sp_A,
               bool trans,
               viennacl::matrix_base<NumericT> & y) {
  ...
  std::string ker_name;
  if(!trans)
    ker_name = "csr_dense_sparse_matrix_mult";
  else
    ker_name = "csr_dense_trans_sparse_matrix_mult";

  viennacl::ocl::kernel &k = ctx.get_kernel(
            viennacl::linalg::opencl::kernels::compressed_matrix<NumericT>::program_name(),
            ker_name);

  viennacl::ocl::enqueue(k(sp_A.handle1().opencl_handle(), sp_A.handle2().opencl_handle(), sp_A.handle().opencl_handle(), cl_uint(sp_A.nnz()),
                           viennacl::traits::opencl_handle(d_A),
                           cl_uint(viennacl::traits::start1(d_A)),          cl_uint(viennacl::traits::start2(d_A)),
                           cl_uint(viennacl::traits::stride1(d_A)),         cl_uint(viennacl::traits::stride2(d_A)),
                           cl_uint(viennacl::traits::size1(d_A)),           cl_uint(viennacl::traits::size2(d_A)),
...
*/
template< typename NumericT, unsigned int AlignmentV>
void copy_impl(const viennacl::compressed_matrix<NumericT, AlignmentV> & sp_A,
               viennacl::matrix_base<NumericT> & y) {
