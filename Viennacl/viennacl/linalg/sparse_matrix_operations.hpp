...
// copy(A,B)(Author: Sangkyun Lee)
/** @brief Carries out matrix-matrix multiplication first matrix being sparse
*
* Implementation of the convenience expression result = prod(d_mat, sp_mat);
*
...
copy_impl(const SparseMatrixType & sp_mat,
          viennacl::matrix_base<ScalarType> & result)
{
    assert( (sp_mat.size1() == result.size1()) && bool("Size check failed for dense - compressed matrix product: size1(sp_mat) != size1(result)"));
    assert( (sp_mat.size2() == result.size2()) && bool("Size check failed for dense - compressed matrix product: size2(sp_mat) != size2(result)"));

    switch (viennacl::traits::handle(sp_mat).get_active_handle_id())
    {
        case viennacl::MAIN_MEMORY:
            //viennacl::linalg::host_based::prod_impl(sp_mat, d_mat, result);
            throw memory_exception("CPU code is not implemented");
            break;
#ifdef VIENNACL_WITH_OPENCL
        case viennacl::OPENCL_MEMORY:
              viennacl::linalg::opencl::copy_impl(sp_mat, result);
              break;
#endif
#ifdef VIENNACL_WITH_CUDA
        case viennacl::CUDA_MEMORY:
      //viennacl::linalg::cuda::prod_impl(sp_mat, d_mat, result);
      throw memory_exception("CUDA code is not implemented");
      break;
#endif
    ...
    */
                    ...
                    viennacl::matrix_base<ScalarType> & result)
    {
      ...
      */
      template<typename SparseMatrixType, class ScalarType>
      typename viennacl::enable_if< viennacl::is_any_sparse_matrix<SparseMatrixType>::value>::type
      prod_impl(const viennacl::matrix_base<ScalarType> & d_mat,
                const SparseMatrixType & sp_mat,
                bool trans,
                viennacl::matrix_base<ScalarType> & result)
      {
          assert( (d_mat.size1() == result.size1()) && bool("Size check failed for dense - compressed matrix product: size1(d_mat) != size1(result)"));
          assert( (d_mat.size2() == sp_mat.size1()) && bool("Size check failed for dense - compressed matrix product: size2(d_mat) != size1(sp_mat)"));
...
#ifdef VIENNACL_WITH_OPENCL
              case viennacl::OPENCL_MEMORY:
                  viennacl::linalg::opencl::prod_impl(d_mat, sp_mat, trans, result);
                  break;
#endif
#ifdef VIENNACL_WITH_CUDA
              case viennacl::CUDA_MEMORY:
