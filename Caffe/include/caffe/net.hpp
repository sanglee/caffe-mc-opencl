...
class Net {
 public:
  ...
  /// @brief Apply L1 Proximal Operator to the updated weights.
  void Proximal(Dtype lambda);

  /// @brief Compress weight matrix and remove uncompressed weight matrix.
  void Packing();

  /// @brief Calculate sparsity for every layer(sparsity = zero weight/total weight).
  Dtype Sparsity();
  /**
 ...
 protected:
  ...
  vector<int> learnable_layer_index_;
  ...
  vector<viennacl::compressed_matrix<Dtype>*> sp_weights_;
  vector<viennacl::matrix<Dtype>*> unpacked_weights_;
  /**
