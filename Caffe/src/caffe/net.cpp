...
void Net<Dtype>::Init(const NetParameter& in_param) {
  ...
  for (int_tp layer_id = 0; layer_id < param.layer_size(); ++layer_id) {
    ...
    for (int_tp param_id = 0; param_id < num_param_blobs; ++param_id) {
      if(layer_param.type() == "InnerProduct"&&param_id == 0){
        this->sp_weights_.push_back(layers_[layer_id]->sp_weight());
        this->unpacked_weights_.push_back(layers_[layer_id]->unpacked_weight());
      }
      this->learnable_layer_index_.push_back(layer_id);
...
void Net<Dtype>::ShareTrainedLayersWith(const Net* other) {
  ...
  for (int_tp i = 0; i < num_source_layers; ++i) {
    ...
    vector<shared_ptr<Blob<Dtype> > >& target_blobs = layers_[target_layer_id]
        ->blobs();
    if(other->layers()[i]->type() == "InnerProduct" && other->layers()[i]->init()){

      unsigned int N = other->layers()[i]->sp_weight()->size1();
      unsigned int K = other->layers()[i]->sp_weight()->size2();

      matrix<Dtype> cpu_weight(N,K);
      memset(&cpu_weight(0,0), 0, sizeof(Dtype)*N*K);
      viennacl::copy(*(other->layers()[i]->sp_weight()),cpu_weight);
      viennacl::copy(cpu_weight,
                     *(this->layers_[target_layer_id]->sp_weight()));

    }
    CHECK_EQ(target_blobs.size(), source_layer->blobs().size())
        << "Incompatible number of blobs for layer " << source_layer_name;
    for (int_tp j = 0; j < target_blobs.size(); ++j) {
      if(other->layers()[i]->type() != "InnerProduct" || j==1)
      {
...
void Net<Dtype>::CopyTrainedLayersFrom(const NetParameter& param) {
  ...
  for (int_tp i = 0; i < num_source_layers; ++i) {
    ...
    for (int_tp j = 0; j < target_blobs.size(); ++j) {
      ...
      if((source_layer.type() == "InnerProduct"||source_layer.type() == "Convolution") && j  == 0){
        Dtype elements[source_layer.elements_size()];
        int row_jumper[source_layer.row_jumper_size()];
        int col_index[source_layer.col_index_size()];
        for(int q = 0;q<source_layer.elements_size();q++){
          elements[q] = source_layer.elements(q);
        }
        for(int q = 0;q<source_layer.row_jumper_size();q++){
          row_jumper[q] = source_layer.row_jumper(q);
        }
        for(int q = 0;q<source_layer.col_index_size();q++){
          col_index[q] = source_layer.col_index(q);
        }
        for(int q=0;q<target_blobs[0]->count();q++){
          target_blobs[0]->mutable_cpu_data()[q] = 0;
        }
        int cnt = 0;
        int q = 0;
        int history = -1;
        int row_index[source_layer.elements_size()];
        int data_index = 0;
        for(int row=0;row<source_layer.row_jumper_size();row++){
          while(data_index<row_jumper[row]){
            row_index[data_index] = row-1;
            data_index++;
          }
        }
        int cntt = 0;
        if(source_layer.type() == "Convolution"){
          matrix<Dtype> cpu_weight(target_blobs[0]->num()*target_blobs[0]->height(),target_blobs[0]->channels()*target_blobs[0]->width());
          memset(&cpu_weight(0,0), 0, sizeof(Dtype)*cpu_weight.size1()*cpu_weight.size2());
          for(int qq=0;qq<source_layer.elements_size();qq++){
            cpu_weight(row_index[qq],col_index[qq]) = elements[qq];
          }

          for(int a=0;a<target_blobs[0]->num();a++){
            for(int b=0;b<target_blobs[0]->channels();b++){
              for(int c=0;c<target_blobs[0]->height();c++){
                for(int d=0;d<target_blobs[0]->width();d++){
                  target_blobs[0]->mutable_cpu_data()[((a * target_blobs[0]->channels() + b) *
                                                       target_blobs[0]->height() + c) * target_blobs[0]->width() + d] =
                          cpu_weight(a*target_blobs[0]->height()+c, b*target_blobs[0]->width()+d);
                }
              }
            }
          }
          ...
          for(int qq=0;qq<source_layer.elements_size();qq++){
            target_blobs[0]->mutable_cpu_data()[row_index[qq]*source_layer.size2()+col_index[qq]] = elements[qq];
          }
      }

      else if((source_layer.type() == "InnerProduct"||source_layer.type() == "Convolution") && j  == 1){
        target_blobs[1]->FromProto(source_layer.blobs(j), kReshape);
      }
      else target_blobs[j]->FromProto(source_layer.blobs(j), kReshape);
    }
  }
}
...
void Net<Dtype>::Update() {
  int fc_count=0;
  for (int_tp i = 0; i < learnable_params_.size(); ++i) {
    if (layers_[learnable_layer_index_[i]]->type() == "InnerProduct") {

      if (i % 2 == 0) {
        greentea_gpu_matrix_axpy(device_->id(), learnable_params_[i]->count(), Dtype(-1),
                            (cl_mem)(learnable_params_[i]->gpu_diff()), 0,
                            (cl_mem)(learnable_params_[i]->mutable_gpu_data()),
                            0, this->unpacked_weights_[fc_count]);
        fc_count++;
      }
      else learnable_params_[i]->Update();
    }
    else learnable_params_[i]->Update();
  }
}
...
template <typename Dtype>
void Net<Dtype>::Proximal(Dtype lambda){

  lambda = log(lambda);
  for(int i=0;i<layers_.size();i++){
    if(layers_[i]->type() == "InnerProduct"||layers_[i]->type() == "Convolution"){
      if(layers_[i]->type() == "Convolution"){
        Dtype* weight_vector = layers_[i]->blobs()[0]->mutable_gpu_data();
        greentea_vector_prox(device_->id(),
                             (cl_mem)weight_vector,
                             layers_[i]->blobs()[0]->count(),
                             lambda);
      }
      else{
        greentea_matrix_prox(device_->id(), layers_[i]->unpacked_weight(), lambda);
      }
...
template <typename Dtype>
void Net<Dtype>::Packing() {
  for (int i = 0; i < layers_.size(); i++) {
    if (layers_[i]->type() == "InnerProduct") {
      compress_gpu_matrix(device_->id(), layers_[i]->unpacked_weight(),
                          layers_[i]->sp_weight());
    }
  }
}
template <typename Dtype>
Dtype Net<Dtype>::Sparsity(){
  Dtype zero_count = 0;
  Dtype total_count = 0;
  for(int i=0;i<layers_.size();i++){
    if(layers_[i]->type() == "Convolution"){
      const Dtype* weight_vector = layers_[i]->blobs()[0]->cpu_data();
      total_count+=layers_[i]->blobs()[0]->count();
      for(int j=0;j<layers_[i]->blobs()[0]->count();j++){
    ...
    else if(layers_[i]->type() == "InnerProduct"){
      total_count+=layers_[i]->sp_weight()->size1()*layers_[i]->sp_weight()->size2();
      zero_count+=(layers_[i]->sp_weight()->size1()*layers_[i]->sp_weight()->size2()-layers_[i]->sp_weight()->nnz());
    }
  }
