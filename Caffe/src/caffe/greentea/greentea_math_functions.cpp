...
template<typename Dtype>
void greentea_gpu_spmm(const int_tp ctx_id, const CBLAS_TRANSPOSE TransA,
                       const CBLAS_TRANSPOSE TransB, const int_tp M,
                       const int_tp N, const int_tp K, const Dtype alpha,
                       const cl_mem A, const int_tp offA,
                       const int_tp offB, const Dtype beta, cl_mem C,
                       const int_tp offC, viennacl::compressed_matrix<Dtype> * scr,
                       viennacl::matrix<Dtype> * unpacked) {
  viennacl::ocl::context &ctx = viennacl::ocl::get_context(ctx_id);
...
//                                  beta);
    ...
    if (TransA == CblasNoTrans && TransB == CblasTrans)
      viennacl::linalg::prod_impl(matA, *scr, true, matC);

    //backward(FC layer)
    else if (TransA == CblasNoTrans && TransB == CblasNoTrans){
      unpacked->resize(B_size1,B_size2);
      viennacl::copy_impl(*scr, *unpacked);
      viennacl::linalg::prod_impl(matA, *unpacked, matC,alpha,beta);
    }
...
template void greentea_gpu_spmm<float>(const int_tp ctx_id,
                                       const CBLAS_TRANSPOSE TransA,
                                       const CBLAS_TRANSPOSE TransB,
                                       const int_tp M, const int_tp N,
                                       const int_tp K, const float alpha,
                                       const cl_mem A, const int_tp offA,
                                        const int_tp offB,
                                       const float beta, cl_mem C,
                                       const int_tp offC, viennacl::compressed_matrix<float> * scr,
                                       viennacl::matrix<float> * unpacked);
template void greentea_gpu_spmm<double>(const int_tp ctx_id,
                                        const CBLAS_TRANSPOSE TransA,
                                        const CBLAS_TRANSPOSE TransB,
                                        const int_tp M, const int_tp N,
                                        const int_tp K, const double alpha,
                                        const cl_mem A, const int_tp offA,
                                         const int_tp offB,
                                        const double beta, cl_mem C,
                                        const int_tp offC, viennacl::compressed_matrix<double> * scr,
                                        viennacl::matrix<double> * unpacked);
...
template<typename Dtype>
void greentea_gpu_matrix_axpy(const int_tp ctx_id, const int_tp N, const Dtype alpha,
                       const cl_mem X, const int_tp offX, cl_mem Y,
                       const int_tp offY, viennacl::matrix<Dtype> *weight) {
  viennacl::ocl::context &ctx = viennacl::ocl::get_context(ctx_id);
  ...
  if (ctx.devices()[0].type() == CL_DEVICE_TYPE_CPU) {
    ...
    viennacl::matrix_base<Dtype, size_t, ptrdiff_t> diffmat(X, ctx, size_type(weight->size1()),
                                                       size_type(0),
                                                       difference_type(1),
                                                       size_type(weight->size1()), size_type(weight->size2()),
                                                       size_type(0),
                                                       difference_type(1),
                                                       size_type(weight->size2())
                                                       VCL_ROW_MAJOR);
    *weight-=diffmat;
  }
}

template void greentea_gpu_matrix_axpy<float>(const int_tp ctx_id, const int_tp N,
                                       const float alpha, const cl_mem X,
                                       const int_tp offX, cl_mem Y,
                                       const int_tp offY,viennacl::matrix<float> *weight);
template void greentea_gpu_matrix_axpy<double>(const int_tp ctx_id, const int_tp N,
                                        const double alpha, const cl_mem X,
                                        const int_tp offX, cl_mem Y,
                                        const int_tp offY,viennacl::matrix<double> *weight);
...
template<typename Dtype>
void greentea_vector_prox(const int_tp ctx_id, cl_mem A, int N, Dtype lambda)
{
      viennacl::ocl::context &ctx = viennacl::ocl::get_context(ctx_id);
      viennacl::vector_base<Dtype, size_t, ptrdiff_t> v1(A, N,
                                                     0,
                                                     1, ctx);

      viennacl::linalg::opencl::l1_prox(v1,
              viennacl::vector_expression<const viennacl::vector_base<Dtype>,
                      const viennacl::vector_base<Dtype>,
                              viennacl::op_element_unary<viennacl::op_prox>>(v1,v1)
                              ,lambda);

}
template void greentea_vector_prox <float> (const int_tp ctx_id, cl_mem A, int N, float lambda);
template void greentea_vector_prox <double> (const int_tp ctx_id, cl_mem A, int N, double lambda);

template<typename Dtype>
void greentea_matrix_prox(const int_tp ctx_id,
        viennacl::matrix<Dtype> *gpu_matrix, Dtype lambda)
{
    viennacl::linalg::opencl::l1_prox2(*gpu_matrix,
    viennacl::matrix_expression<const viennacl::matrix<Dtype>,
                const viennacl::matrix<Dtype>,
                viennacl::op_element_unary<viennacl::op_prox>>
                (*gpu_matrix,*gpu_matrix),lambda);
    return;
}
template void greentea_matrix_prox <float> (const int_tp ctx_id,
        viennacl::matrix<float> *gpu_matrix, float lambda);
template void greentea_matrix_prox <double> (const int_tp ctx_id,
        viennacl::matrix<double> *gpu_matrix, double lambda);

template<typename Dtype>
void copy_array_to_csr(const int_tp ctx_id, const Dtype * org,
        viennacl::compressed_matrix<Dtype> *csr_matrix,
        unsigned int N_,
        unsigned int K_) {

    viennacl::ocl::context &ctx = viennacl::ocl::get_context(ctx_id);
    csr_matrix->switch_memory_context(ctx);

    matrix<Dtype> cpu_weight (N_, K_);
    memset(&cpu_weight(0,0), 0, sizeof(Dtype)*N_*K_);
    for(int i=0;i<N_;i++) {
        for(int j=0;j<K_;j++){
          cpu_weight(i,j) = org[K_*i+j];
        }
    }
    viennacl::copy(cpu_weight,*csr_matrix);
}
template void copy_array_to_csr<float>(const int_tp ctx_id, const float * org,
        viennacl::compressed_matrix<float> *csr_matrix,
        unsigned int N_,
        unsigned int K_);

template void copy_array_to_csr<double>(const int_tp ctx_id, const double * org,
        viennacl::compressed_matrix<double> *csr_matrix,
        unsigned int N_,
        unsigned int K_);


template<typename Dtype>
void compress_gpu_matrix(const int_tp ctx_id, viennacl::matrix<Dtype> * org_matrix,
        viennacl::compressed_matrix<Dtype> *csr_matrix) {

  viennacl::ocl::context &ctx = viennacl::ocl::get_context(ctx_id);
  csr_matrix->switch_memory_context(ctx);

  int M = org_matrix->size1();
  int N = org_matrix->size2();

  matrix<Dtype> cpu_matrix (M, N);
  memset(&cpu_matrix(0,0), 0, sizeof(Dtype)*M*N);

  viennacl::copy(*org_matrix,cpu_matrix);
  viennacl::copy(cpu_matrix,*csr_matrix);

  org_matrix->resize(1,1);

}
template void compress_gpu_matrix<float>(const int_tp ctx_id,
        viennacl::matrix<float> * org_matrix,
        viennacl::compressed_matrix<float> *csr_matrix);
template void compress_gpu_matrix<double>(const int_tp ctx_id,
        viennacl::matrix<double> * org_matrix,
        viennacl::compressed_matrix<double> *csr_matrix);

template<typename Dtype>
void copy_array_to_csr_4d(const Dtype * org,
        viennacl::compressed_matrix<Dtype> *csr_matrix,
        unsigned int size1,
        unsigned int size2,
        unsigned int size3,
        unsigned int size4) {
    matrix<Dtype> cpu_weight (size1*size3,size2*size4);
    memset(&cpu_weight(0,0), 0, sizeof(Dtype)*size1*size2*size3*size4);

    for(int i=0;i<size1;i++) {
      for (int j = 0; j < size2; j++) {
        for (int p = 0; p < size3; p++) {
          for (int q = 0; q < size4; q++) {
              cpu_weight(i*size3+p, j*size4+q)=org[((i * size2 + j) * size3 + p) * size4 + q];
          }
        }
      }
    }
...
template void copy_array_to_csr_4d<float>(const float * org,
        viennacl::compressed_matrix<float> *csr_matrix,
        unsigned int size1,
        unsigned int size2,
        unsigned int size3,
...
template void copy_array_to_csr_4d<double>(const double * org,
        viennacl::compressed_matrix<double> *csr_matrix,
        unsigned int size1,
        unsigned int size2,
        unsigned int size3,
