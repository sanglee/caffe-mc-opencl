...
void Blob<Dtype>::Remove_data() {
  data_.reset(new SyncedMemory(1 * sizeof(Dtype), device_,
                               dtypeof<Dtype>()));
}

template<typename Dtype>
void Blob<Dtype>::Restore_data() {
...
template <>
void Blob<double>::ToProtoOnlyDiff(BlobProto* proto, bool write_diff) const {
  proto->clear_shape();
  for (int_tp i = 0; i < shape_.size(); ++i) {
    proto->mutable_shape()->add_dim(shape_[i]);
  }
...
template <>
void Blob<float>::ToProtoOnlyDiff(BlobProto* proto, bool write_diff) const {
  proto->clear_shape();
  for (int_tp i = 0; i < shape_.size(); ++i) {
    proto->mutable_shape()->add_dim(shape_[i]);
  }
