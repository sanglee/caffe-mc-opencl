...
void InnerProductLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  /**fill compressed weight matrix if it is the first forward operation.*/
  if(!this->init_){
    const Dtype* cpu_weight = this->blobs_[0]->cpu_data();
    copy_array_to_csr(this->device_->id(), cpu_weight, &(this->sp_weight_), N_, K_);
    this->init_ = true;

    this->blobs_[0]->Remove_data();
  ...
  if (this->device_->backend() == BACKEND_CUDA) {
    ...
    if (M_ == 1) {
      ...
      greentea_gpu_spmm<Dtype>(this->device_->id(), CblasNoTrans,
                               transpose_ ? CblasNoTrans : CblasTrans,
                               M_, N_, K_, (Dtype) 1.,
                               (cl_mem) bottom_data, 0, 0,
                               (Dtype) 0., (cl_mem) top_data, 0, &(this->sp_weight_),&(this->unpacked_weight_));
      if (bias_term_)
    ...
    if (propagate_down[0]) {
      ...
      if (transpose_) {
        ...
        greentea_gpu_spmm<Dtype>(this->device_->id(), CblasNoTrans,
                                 ...
                                 (cl_mem) top_diff, 0, 0,
                                 (Dtype) 0.,
                                 (cl_mem) (bottom[0]->mutable_gpu_diff()),
                                 0, &(this->sp_weight_),&(this->unpacked_weight_));
      }
    }
#endif  // USE_GREENTEA
  }
